...
function BannerSection({ banner }: { banner: any }) {
    return (
        <div className="relative w-full">
            <Image src={images.navbarLogo} alt="bg" className="h-[80vh] w-full object-cover object-left" width={1920} height={1080} />
            <section className="absolute top-1/2 right-1/2 md:right-1/4 w-[80%] md:w-1/2 transform translate-x-1/2 -translate-y-1/2 text-left flex flex-col gap-2 text-black items-baseline">
                ...
                <a href={banner.buttonRedirecting.url} className="group relative inline-flex items-center overflow-hidden bg-awblack p-3 pr-0 hover:pr-3 text-awgraylight transition-all duration-300 hover:bg-awpink hover:text-black">
            ...
            <div className="absolute right-1/2 md:right-1/6 transform translate-x-1/2 top-1/5 -translate-y-1/2">
                <div className="relative w-1/2 h-1/2">
                    <Image src={images.pub} alt="publicité" className="w-full h-full object-cover animate-spin-slow" width={400} height={400} />
                    <Image src={images.pubLogo} alt="logo de la publicité" className="absolute inset-0 m-auto w-1/2" width={200} height={200} />
                </div>
            </div>
        </div>
    );
...
function ArticleSection({ articles }: { articles: any[] }) {
    return (
        <div className="relative w-full">
            <section className="relative w-full flex flex-col ml-auto mr-auto md:flex-row md:justify-between md:w-[80%] p-16">
                {articles.map((a, i) => (
                    <article key={a.id} className="relative flex flex-col gap-2 text-left text-black w-full md:w-1/4 overflow-hidden md:overflow-visible">
